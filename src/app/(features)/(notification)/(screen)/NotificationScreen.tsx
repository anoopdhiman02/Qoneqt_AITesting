...
import { AnyApp } from "realm/dist/public-types/namespace";
import { useScreenTracking } from "@/customHooks/useAnalytics";
import { useDispatch } from "react-redux";
import { onDeleteAllNotification, onDeleteNotification } from "@/redux/reducer/notification/DeleteNotification";
import { clearData, updateNotificationData } from "@/redux/slice/notification/FetchNotificationSlice";
...
const FooterLoader = () => (
  <View style={{ paddingVertical: 20 }}>
    <ActivityIndicator size="large" color={globalColors.darkOrchidShade60} />
  </View>
);
...
const NotificationScreen = () => {
    ...
    switch (screen) {
          ...
          case "loop_join":
            router.push({
              ...
              params: { slug: remoteMessage?.loop_id || remoteMessage?.redirect_id || "india" },
            });
            break;
          case "personal_chat":
            router.push({
          ...
          case "profile":
            router.push({
              pathname: "/profile/[id]",
              params: { id: remoteMessage?.profile_id || remoteMessage?.redirect_id || "2", isProfile: "true" },
            });
            break;
          case "user chat":
            router.push({
          ...
          case "profile_follow":
            router.push({
              pathname: "/profile/[id]",
              params: {
                id: remoteMessage?.profile_id || remoteMessage?.post_id || remoteMessage?.redirect_id || "2",
                isProfile: "true",
              },
            });
            break;
          ...
          case "kyc_declined":
            router.push({
              pathname: "/groups/[slug]",
              params: {
                slug: remoteMessage?.loop_id || remoteMessage?.redirect_id || "india",
              },
            });
            break;
          ...
          case "group_join":
            router.push({
              pathname: "/groups/[slug]",
              params: { slug: remoteMessage?.group_slug || remoteMessage?.redirect_id || "india" },
            });
            break;
          case "message":
            router.push({
  ...
  const deleteNotificationData = (id: number) => {
    ...
    const newData = {
      allData: notificationsData?.allData?.filter((item) => item.id != id),
      postData: notificationsData?.postData?.filter((item) => item.id != id),
      groupData: notificationsData?.groupData?.filter((item) => item.id != id)
    };
  ...
  const renderItem = useCallback(({ index,item }) => {
    ...
    return (
      ...
      onDelete={()=>{deleteNotificationData(item.id)}}
      itemId={item.id}
      // disabled={disabled}
      >
      <View key={index}>
        <View
          ...
          >
            <TouchableOpacity
              ...
              onPress={() => {
                item?.sender?.id === userId
                  ...
                  : router.push({
                      pathname: "/profile/[id]",
                      params: { id: item?.sender?.id, isProfile: "true", isNotification: "false" },
                    });
              }}
            >
            ...
            >
              ...
              {item?.notification_content && (<Text
                numberOfLines={1}
                style={{
                  fontSize: 14,
                  letterSpacing: -0.1,
  ...
  const flatListData = useMemo(() => {
    ...
    //@ts-ignore
    const sortedNotifications = [...notfyData].sort((a, b) => new Date(b.created_at) - new Date(a.created_at)
    );
