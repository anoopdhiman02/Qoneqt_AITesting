...
interface commentShow {
  show_Button: boolean; 
  setShow_Button: (update: boolean | ((prev: boolean) => boolean)) => void;
}
export const useCommentShow = create<commentShow>((set) => ({
  show_Button: true, 
  setShow_Button: (update) =>
    set((state) => ({
      show_Button:
        typeof update === "function" ? update(state.show_Button) : update, 
    })),
}));
