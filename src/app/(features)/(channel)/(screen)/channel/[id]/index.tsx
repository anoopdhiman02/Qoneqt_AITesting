...
const ChannelDetailScreen = () => {
  ...
  const { channelId, channelDetails, userChannelRole } = useChannelStore();
  const { updateUserData } = UserStoreDataModel();

  const { channelData, onFetchChannelInfoHandler, loading } =
  ...
  useEffect(() => {
    if (params?.id) {
      onFetchChannelInfoHandler({ channelId: params?.id });
    }
    updateUserData();
    return () => {
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                #{channelData?.channel_name||"Qoneqt"}
              </Text>
