...
const DashboardScreen = () => {
  ...
  const newData = getPrefsValue("message");
  const { OnRefreshHandlerHome } = useHomeViewModel();
  ...
  const { show_Button } = useCommentShow();
  const [postGroupData, setPostGroupData] = useState("");
  const { commentRef, profileRef, blockUserRef } = useBottomSheetRefs();
  ...
  useEffect(() => {
    if (scrollViewRef.current) {
      //  scrollViewRef.current.scrollTo({ animated: false, offset: 0 });
      scrollViewRef.current.scrollToOffset({ offset: 0, animated: true });
      //  scrollViewRef.current.scrollToOffset(0, 0, true);
    }
  }, [Selectedtab]);
    ...
    const backHandler = BackHandler.addEventListener(
      ...
      () => {
        if (router.canGoBack()) {
          ...
          if (isComment || commentRef.current) {
            setIsComment(false);
  ...
  const handleCreatePostGroup = () => {
    setPostValue({})
    router.push(Selectedtab > 1 ? "/CreateGroupScreen" : "/CreatePostScreen");
  };
  ...
  const debouncedEndReached = async () => {
    try {
      ...
      if (Selectedtab !== 1 && !homePostResponse.isLoaded && isFocused) {
        setIsNextPostLoading(true);
        // If last_count is same, append nextPostData and refetch
  ...
  const onViewableItemsChanged = useCallback(({ viewableItems }: any) => {
    ...
    const postsToTriggerAt = Math.floor(lastVisibleIndex % 8);
    if (postsToTriggerAt == 0 && lastVisibleIndex > 5) {
      Newfeed();
    }
  }, []);
  ...
  const onKycPress = async () => {
    ...
    const kycStatus = userDetailsData?.data?.kyc_status;
    const stepStatus = Number(userDetailsData?.data?.kyc_details?.id_verification_status);
  ...
  return (
    <ViewWrapper onPress={handleOnPress}>
      ...
      <View style={Styles.container}>
        ...
        {/* Step Modal */}
        {isFlex ? renderHomeView() : renderGroupView()}
        {isFlex && show_Button && (
          <View
            style={{ justifyContent: "flex-end", top: showKycBtn ? -40 : 0 }}
          >
            <FloatingButton
              onPress={handlePress}
              size={45}
