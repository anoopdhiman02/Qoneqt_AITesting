...
const PostDetailScreen = () => {
  ...
  const postData = params?.postData
    ? JSON.parse(params?.postData as string)
    : postDetailsResponse?.newData?.id
  ...
  const renderPostItem = useCallback(({ index, islocal }) => {
    if (index == 0) {
      return (
        <View>
          <TopPostComponent
          profilePic={postData?.post_by?.profile_pic}
          name={postData?.post_by?.full_name}
      ...
      return (
        <PostBottomComponent
          postId={postData?.id}
          likeCount={
            (postData?.likes_aggregate?.aggregate?.count || 0) + (postData?.like_count || 0)
          }
          isLiked={(postData?.like_byme||postData?.like_ByMe || postData?.likeByMe)?.length > 0 ? 1 : 0}
        />
      );
    }
  }, [postData]);
