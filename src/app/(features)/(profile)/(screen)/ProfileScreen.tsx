...
const MemoizedPostLoader = React.memo(PostLoaderComponent);
...
const ProfileScreen = () => {
  ...
  const onKycPress = useCallback(async () => {
    ...
    try {
      ...
      const stepStatus = Number(kycData?.id_verification_status || 0);
      const stepType = kycData?.identification_type;

      const handleKycCompletion = async (path: string, params = {}) => {
  ...
  const onPressCommentHandler = useCallback((postId: string, userId: string) => {
