...
console.log("updatedLoginData>>",  postDetailResponse.updatedData);
      // Update profile screen data
      const updateProfileScreenData = (postDetailResponse?.updatedData.length == 0 ? []: postDetailResponse?.updatedData[0]?.post_by ? postDetailResponse?.updatedData : [] ).map(
        (item: any) =>
          {
            if(item?.post_by?.id){
              return {
    ...
    setPicLoading(false);
    console.log("updatePictureData", err?.message);
